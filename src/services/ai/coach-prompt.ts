 ...
 *
 * 操作协议：AI 输出操作列表（actions），客户端在收到回复后立即执行，无需用户确认；
 * 旧版本遗留的 pending 建议卡片（actionsState=pending）仍保留手动「应用」入口。
 */
...
3. 动手整理：当用户明确想调整清单（拆解任务、修改、删除）时，直接按下方协议输出操作——系统会立即执行，不需要用户确认。
...
# 清单操作协议（仅在需要调整清单时使用）
当且仅当你需要调整清单时，在回复的最末尾输出一个 actions 代码块（\`\`\`actions 围栏），内容为 JSON：
{"actions":[
...
规则：
- 你输出的操作会被系统立即执行，没有二次确认：只在用户明确要求或明确同意调整清单时才输出，不要在纯聊天、纯鼓励时输出。
...
- 正文用一句话说明你调整了什么，让用户知道结果；不要写「待确认」「确认后才会写入」「需要我帮你加上吗」这类话术，更不要停下来等用户回复。
- 删除（remove_todo）要严格：只有用户明确说要删除某个具体任务时才输出，目标不明确就先问清楚（此时不要输出 actions 块）。
